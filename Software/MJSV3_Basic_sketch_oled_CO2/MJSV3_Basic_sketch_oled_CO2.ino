...
void setup() {
  ...
  if(DEBUG) {
    setup_serial();
    Serial.println(F("Starting Setup2"));
  }
  // If enabled initialise OLED display
  if (OLED) {
    u8g2.begin();
    }
  ...
  if (CO2){
    ...
    delay(100);  // wait for 5V PSU to be stable
    Wire.begin();
   ...
   if (OLED) {
    ...
    u8g2.drawStr(0,10,"Starting Sensor");  // write something to the internal memory
...
void loop() {
   ...
   if (OLED) {
    u8g2.clearBuffer();          // clear the internal memory
    u8g2.setFont(u8g2_font_ncenB08_tr); // choose a suitable font
    u8g2.drawStr(0,10,"Starting Sensor2");  // write something to the internal memory
    u8g2.sendBuffer();          // transfer internal memory to the display
  }
    ...
    if (airSensor.dataAvailable())
  {
    u8g2.clearBuffer();                  // clear the internal memory
    u8g2.setFont(u8g2_font_ncenB08_tr);  // choose a suitable font
    String CO2value = String("co2(ppm): "+ String(airSensor.getCO2() ) );
    String Tvalue   = String("Temp (C): "+ String(airSensor.getTemperature(),1) );
    String RHvalue  = String("RH   (%): "+ String(airSensor.getHumidity(),1) );
    u8g2.setCursor(0, 10);
    u8g2.print(CO2value);
    u8g2.setCursor(0, 25);
    u8g2.print(Tvalue);
